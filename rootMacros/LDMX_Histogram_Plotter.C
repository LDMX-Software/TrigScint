...
void LDMX_Histogram_Plotter(const char *sigfile, const char *identifier)
{
	TFile *hzz = TFile::Open(sigfile);
	TString Default("LDMX_Analysis_Run1/");
	TString Address = Default + identifier;
	TString location_ADC = Address + "/ADC/";
	TString location_Charge = Address + "/Charge/";
	TString location_Hits = Address + "/Hits/";
	...
	TString plot_title_QTot_med("hQTot_med3_chan");
	...
	for(unsigned int iB=0;iB < 16; iB++){
		...
		HistogramPlotterLDMX(hzz,100,PE_low,PE_med1,filename_QTot_med,plot_QTot_med,location_Charge,1);
		HistogramPlotterLDMX(hzz,100,PE_med1,PE_med2,filename_QTot_med2,plot_QTot_med2,location_Charge,1);
