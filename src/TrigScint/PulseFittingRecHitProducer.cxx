#include "TrigScint/PulseFittingRecHitProducer.h"
...
PulseFittingRecHitProducer::PulseFittingRecHitProducer(const std::string &name,
                                                 framework::Process &process)
    : Producer(name, process) {}

PulseFittingRecHitProducer::~PulseFittingRecHitProducer() {}

void PulseFittingRecHitProducer::configure(
...
void PulseFittingRecHitProducer::produce(framework::Event &event) {
  ...
  Double_t PulseFittingRecHitProducer::ChargeReconstruction(std::vector<int>adc
                              ,std::vector<int>tdc
                              ,int sample) {
  ...
  for(int i=0;i<tdc.size();i++) {
    Qdata[i] = qie.ADC2Q(adc[i]); // Linearized charge from i'th TS.
    QErr[i] = qie.QErr(Qdata[i]); // Quantization error
    if(tdc[i]<50) {		  // Make sure TDC is physical
  ...
  if(npulses==0) return 0.0;	// no pulse
...
DECLARE_PRODUCER_NS(trigscint, PulseFittingRecHitProducer);
