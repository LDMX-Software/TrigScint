...
void extractPEPeakIntegrals(string inFile, TString location, int deadChannel=-1, string decodePassName = "hits", int nSamples = 30, int verbosity=2, bool isSim=false, bool doClean=true)
{
  ...
  vector <int> maxVals = {5};  // don't need to go high, focus on the single-few PE peaks 
  ...
  IntegralFile=IntegralFile.ReplaceAll(".root", "_Integral.txt");
  TString IntegralErrorFile = inFile;
  IntegralErrorFile=IntegralErrorFile.ReplaceAll(".root", "_IntegralError.txt");
