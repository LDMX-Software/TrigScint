...
//make a clone where we can iteratively remove what is to the left of peak of interest 
  ...
  float oldMean = hIn->GetXaxis()->GetXmin();
  ...
  while ( max <= maxVal ) {
    ...
    fGausExp->SetParLimits(2,0.01,max-minVal);
  ...
  // }
   TF1 *GausInt = new TF1("GausInt",gaussianPeak,minVal-5*sigma,max+5*sigma,3);
...
void extractPEPeakIntegrals_excludePedestal(string inFile, TString location, int deadChannel=-1, string decodePassName = "hits", int nSamples = 30, int verbosity=2, bool isSim=false, bool doClean=true)
{
  ...
  //TFile * fIn2 = TFile::Open( inFile2.c_str(), "READ");
  //TFile * fIn3 = TFile::Open( inFile3.c_str(), "READ");
  //TChain chain("LDMX_Events");
  //chain.Add(fIn1);
  //chain.Add(fIn2);
  //chain.Add(fIn3);
  ...
  vector <double> maxVals = {6.5};  // don't need to go high, focus on the single-few PE peaks 
  vector <double> minVals = {0.5};  // should cover most negative pedestals 
  vector <float> binFactor = {150.0/6.0}; // histogram binning 
  ...
  for (unsigned int iV = 0; iV < vars.size(); iV++) {
	for (unsigned int iC = 0; iC < cuts.size(); iC++) {
	  string cut =cuts[iC];
	  //set up binning
	  string bins = Form("%i, %f, %f", (int)(binFactor[iV]*(maxVals[iV]-minVals[iV])), minVals[iV], maxVals[iV]);
	  if (iC == 0)
  ...
  TString IntegralFile = inFile;
  IntegralFile=IntegralFile.ReplaceAll(".root", "_Integral_excPed.txt");
  TString IntegralErrorFile = inFile;
  IntegralErrorFile=IntegralErrorFile.ReplaceAll(".root", "_IntegralError_excPed.txt");
